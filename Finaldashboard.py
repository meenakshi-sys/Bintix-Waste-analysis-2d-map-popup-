...
try:
    ...
    CO2_ICON      = load_icon_data_uri("co2_averted.png")

except FileNotFoundError as e:
    st.error(f"{e}\nMake sure your icons are in: {ASSETS_DIR}")
    TREE_ICON = HOUSE_ICON = RECYCLE_ICON = CO2_ICON = ""
    

# ---------------- Data loading (CSV only) ----------------

CSV_WINDOWS_DEFAULT = Path(r"C:\Users\meena\Downloads\kaggle\FINALWORK\book22.csv")

CSV_LOCAL_FALLBACK  = BASE_DIR / "book22.csv"
...
def format_co2_value(val: float) -> str:
    if val >= 1000:
...
with tab_map:
    ...
    cB.metric("CO₂ Averted", f"{format_co2_value(stats['co2'])} kg")
